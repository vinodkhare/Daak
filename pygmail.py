...
class pygmail(object):
	...
	def getUnreadCount(self, folder='Inbox'):
	...
	def getUnreadEmailIds(self):
		status, eids = self.M.search(None, '(UNSEEN)')
		emailIds = eids[0].split(' ')
		return emailIds
		
	def getUnreadHeaderField(self, field):
		fieldList = []
		emailIds = self.getUnreadEmailIds()
		
		# If no unread messages exist then return empty
		if len(emailIds[0]) == 0:
			return fieldList

		for eid in emailIds:
			rc, response = self.M.fetch(eid, '(body[header.fields (%s)])' % field)
			fieldList.append( response[0][1][len(field) + 2:-4] )
